...
public abstract class DAO<T> implements DAOInterface<T> {
	...
	public static void open(){
		if(manager==null){
			...
			if(sgbd.equals("mysql"))
				prop.setProperty("javax.persistence.jdbc.url", "jdbc:mysql://"+ip+":3306/projeto3?createDatabaseIfNotExist=true");
	...
	public static void clear() {
		...
		log.debug("Deletou "+ cont + " assuntos");

		q = manager.createQuery("delete from Visualizacao");
		cont =  q.executeUpdate();
		log.debug("Deletou "+ cont+ " visualizações");

		q = manager.createQuery("delete from Video");
		cont =  q.executeUpdate();
		log.debug("Deletou "+ cont+" videos");

		q = manager.createQuery("delete from Usuario");
		cont =  q.executeUpdate();
		log.debug("deletou "+ cont+ "usuários");
		log.debug("");
